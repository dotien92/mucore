...
if($dato=="lista"){
    ...
    while($posicion < $largo){
        $num_mapa = decoBmd2(1, substr($contenido, $posicion    , 4), 0, 4, 0);
        $cant_txt = decoBmd2(1, substr($contenido, $posicion + 4, 4), 0, 4, 0);
        $r .= "{e:0,i:".$i.",o0:'".$num_mapa."',o1:'".$cant_txt."'},\n";
        $posicion += 8 + $cant_txt * 4;
        $i++;
    }
    echo  '{"datos": ['.substr_replace ($r, '', -2).']}';
}
elseif($dato=="datos"){
    $arrastre = $conf[2] * $id + $conf[3];
    $r    = $id;
    ...
    $r    .= "|".iconv($load_txt_lang, "UTF-8", decoBmd2(0, substr($contenido, $id * $conf[1] + $col[3][2], $col[3][3]), $id * $col[3][1] + $i * $conf[2], $col[3][3], 0));
    echo $r;
}
elseif($dato=="lista2"){
    $r    = "";
