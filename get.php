...
if (isset($_GET['aG'])) {
    ...
    if ($get_config->active == '0') {
        echo json_encode(array('success' => false, 'message' => msg('0', text_sorry_feature_disabled)));
        exit;
    }
    ...
    $name = safe_input(set_limit(trim($name_raw), '10', ''), '\ ');
    $message = htmlspecialchars(stripslashes(set_limit(trim(str_replace("\r", " ", str_replace("\n", " ", str_replace("¦", "", $message_raw)))), '300', '')));

    if (empty($name) || empty($message)) {
        echo json_encode(array('success' => false, 'message' => msg('0', text_chat_error1)));
        exit;
    }

    if (strlen($name) < 4) {
        echo json_encode(array('success' => false, 'message' => msg('0', text_chat_error2)));
        exit;
    }
    ...
    if (!isset($_SESSION['admin_login_auth'])) {
        if (strtolower($name) == strtolower($get_config->admin_name)) {
            echo json_encode(array('success' => false, 'message' => msg('0', text_chat_error3)));
            exit;
        }
    } else {
        if (strtolower($name) == strtolower($get_config->admin_name)) {
    ...
    if (!$chat_db) {
        echo json_encode(array('success' => false, 'message' => msg('0', 'Khong the ghi tin nhan.')));
        exit;
    }

    fwrite($chat_db, $name . "¦" . $message . "¦" . time() . "¦" . $_SERVER['REMOTE_ADDR'] . "¦" . $flag . "¦\n");
    fclose($chat_db);

    echo json_encode(array('success' => true, 'message' => msg('1', 'Tin nhan da duoc gui.'), 'name' => $name));
    exit;
