        ...
        case 'characters':
            if ($rank_class_type == 'all') {
                ...
                if (cron_check($jq_cron->fields[0]) == false) {
                    ...
                    $qry_r      = $core_db->Execute("Select top " . $get_config->char_top . " mu_id,name,class,clevel,resetcount,strength,dexterity,vitality,energy,ctlcode,accountid,leadership,grand_resets from character order by grand_resets desc, resetcount desc, clevel desc");
                    while (!$qry_r->EOF) {
                ...
                if (cron_check($jq_cron->fields[0]) == false) {
                    ...
                    while (!$qry_c_dis->EOF) {
                        $qry_c = $core_db->Execute("Select top " . $get_config->char_top . " mu_id,name,class,clevel,resetcount,strength,dexterity,vitality,energy,ctlcode,accountid,leadership,grand_resets from character where class='" . $qry_c_dis->fields[0] . "' order by grand_resets desc, resetcount desc, clevel desc");
                        while (!$qry_c->EOF) {
